...
namespace fluidClasses
{
    public class UniformGrid : MonoBehaviour
    {
        ...
        public void init(Vector3 force, Vector3 initialPosition, float _minNodeSize, float _resolution, float _height)
        {
            ...
            nodes = new UniformNode[(int)_resolution, (int)_height, (int)_resolution];
        ...
        void createGridNeighbours()
        {
            ...
            for (int x = 1; x < resolution; ++x)
            {
                for (int y = 1; y < height; ++y)
                {
                    ...
                    {
                        // get the node
                        UniformNode node = nodes[x, y, z];
                        
                        // assign each neighbour manually
                        node.surroundingBounds[0] = nodes[x + 1, y + 1, z + 1];
                        node.surroundingBounds[1] = nodes[x + 1, y + 1, z];
                        node.surroundingBounds[2] = nodes[x + 1, y, z];
                        node.surroundingBounds[3] = nodes[x, y + 1, z + 1];
                        node.surroundingBounds[4] = nodes[x, y + 1, z];
                        node.surroundingBounds[5] = nodes[x, y, z + 1];
                        node.surroundingBounds[6] = nodes[x, y + 1, z];
                        node.surroundingBounds[7] = nodes[x - 1, y - 1, z - 1];
                        node.surroundingBounds[8] = nodes[x - 1, y - 1, z];
                        node.surroundingBounds[9] = nodes[x - 1, y, z];
                        node.surroundingBounds[10] = nodes[x, y - 1, z - 1];
                        node.surroundingBounds[11] = nodes[x, y, z - 1];
                        node.surroundingBounds[12] = nodes[x, y - 1, z];
                        node.surroundingBounds[13] = nodes[x - 1, y + 1, z + 1];
                        node.surroundingBounds[14] = nodes[x + 1, y - 1, z + 1];
                        node.surroundingBounds[15] = nodes[x + 1, y + 1, z - 1];
                        node.surroundingBounds[16] = nodes[x - 1, y - 1, z + 1];
                        node.surroundingBounds[17] = nodes[x - 1, y, z + 1];
                        node.surroundingBounds[18] = nodes[x - 1, y + 1, z - 1];
                        node.surroundingBounds[19] = nodes[x - 1, y, z - 1];
                        node.surroundingBounds[20] = nodes[x + 1, y, z + 1];
                        node.surroundingBounds[21] = nodes[x, y - 1, z + 1];
                        node.surroundingBounds[22] = nodes[x, y + 1, z - 1];
                        node.surroundingBounds[23] = nodes[x - 1, y + 1, z];
                        node.surroundingBounds[24] = nodes[x + 1, y, z - 1];
                        node.surroundingBounds[25] = nodes[x + 1, y - 1, z];
                    }
                }
            }
        ...
        public void addParticleToGrid(ParticleClass particle, bool remove)
        {
            ...
            if (remove)
            {
                ...
                UniformNode[] surrounding = nodes[(int)ID.x, (int)ID.y, (int)ID.z].surroundingBounds;
        ...
        public void removeParticle(ParticleClass particle)
        {
            ...
            Vector3 ID = particle.gridID;
            nodes[(int)ID.x, (int)ID.y, (int)ID.z].objectsHeld.Remove(particle);
        }
        ...
        public void getNeighbours(ref int[] neighbours, ParticleClass particle, float smoothingDistance, float maxInteractions, float cellspace2Sq)
        {
            ...
            nodes[(int)ID.x, (int)ID.y, (int)ID.z].getNeighbours(ref neighbours, particle, smoothingDistance, maxInteractions, cellspace2Sq);
